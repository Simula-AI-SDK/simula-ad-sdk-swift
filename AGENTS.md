...
- **WebView pool**: `@MainActor`; consent-aware data store; release = stop loading, nil delegates, `about:blank`; script handlers use the stable-forwarder pattern (never re-register per acquire, never leak on discard). *Startup* prewarm skips at/below a 2 GiB memory floor (`WebViewPrewarmPolicy`); normal pooling after an ad request is never gated.
- **Background flush**: `TelemetryBackgroundFlush` (installed once at telemetry init) persists + delivers buffered telemetry on `didEnterBackground`; `flushNow` is idempotent.
