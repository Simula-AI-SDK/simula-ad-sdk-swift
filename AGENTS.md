...
Host app
  → SimulaProviderView (SwiftUI) | SimulaAds.initialize (imperative)
    → SimulaProvider.init        ← the single init path (cheap, main-thread):
                                     ...
                                     durable queue recovery, version check →
                                     session warm-up). Ad load/preload requests remain
                                     network-only; successful fullscreen loads may schedule a
                                     launch-gated ready-state WebView prewarm after publisher
                                     callbacks. Native loads and startup never prewarm WebViews;
                                     otherwise visible UI creates them. `ensureSession`
...
- **Launch quiet window**: `LaunchSettledGate.shared` is one non-blocking five-second gate for telemetry sends/recovery replay, crash replay, startup-triggered durable queue drains, IPv4 beacons, and deferred ATT/IDFA reads. Live beacon/reward enqueues persist and drain without awaiting it; only `triggerProcessQueue()` startup recovery waits, without claiming queue processing while blocked. Session creation and ad load never await it. Engine tests inject `ImmediateLaunchSettledGate` or a controllable gate.
- **Durable billing queues**: beacon/reward enqueue state and atomic Application Support persistence run on each manager's utility serial executor, never on the UI caller and never under an `NSLock`. Load/save failures block delivery; bounded single-task load, persistence, and network retries preserve and later merge deduped in-memory enqueues without overwriting unreadable current files. Malformed obsolete legacy defaults are cleared at their exact keys; valid migration from `simula_pending_beacons` and `simula_pending_reward_verifications` remains one-time.
...
- **WebView pool**: `@MainActor`; one idle view (zero on constrained devices), five-minute cooldown only for memory pressure/renderer death (background only clears idle views), prewarm only from an already-open minigame UI or after a successful interstitial/rewarded load has notified the publisher, awaited `LaunchSettledGate.shared`, confirmed that serve is still ready, and confirmed that no SDK fullscreen presentation is active; native-ad load/preload and unconditional startup never prewarm WebViews, and acquire never refills; consent-aware data store; release = stop loading, nil delegates, `about:blank`; script handlers use the stable-forwarder pattern (never re-register per acquire, never leak on discard).
...
Tests in `Tests/SimulaAdSDKTests/`, XCTest with `@testable import`. Tier 0: pure functions (parsing, classify, backoff). Tier 1: engines with `FakeStore`/`FakeSender`, temporary file URLs, and isolated `UserDefaults(suiteName: UUID)` for migration only. Inject `now`, `random`, `backoff`, launch gates, and platform readers for determinism. No real network.
